...
const { searchYouTubeAPI, isConfigured: isYouTubeAPIConfigured } = require('./api.service');
...
async function executeSearchPlayDl(query, expectedTitle, expectedArtist) {
    try {
        ...
        const videoResults = searchResults.filter(result => {
            ...
            let urlType = youtubeCache.getValidation(result.url);
            if (!urlType) {
                urlType = play.yt_validate(result.url);
                youtubeCache.setValidation(result.url, urlType);
            }
...
async function executeSearch(query, expectedTitle, expectedArtist, preferAPI = true) {
    ...
    if (preferAPI && isYouTubeAPIConfigured() && quotaService.hasQuotaAvailable()) {
        try {
    ...
    youtubeCache.setSearch(query, scoredResults);
...
async function searchYouTube(query, options = {}) {
    ...
    const useAPI = isYouTubeAPIConfigured() && quotaService.hasQuotaAvailable();
    ...
    let urlType = youtubeCache.getValidation(bestResult.url);
    if (!urlType) {
        urlType = play.yt_validate(bestResult.url);
        youtubeCache.setValidation(bestResult.url, urlType);
    }
    if (urlType !== 'video') {
