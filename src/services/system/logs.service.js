...
class LogsService extends EventEmitter {
     ...
     * @param {string} message - Log message (may include JSON context)
     * @param {string} source - Source file/function
     * @param {Object} context - Optional structured context object
     */
    addLogDirect(level, message, source, context = null) {
        // Parse context from message if it contains JSON
        ...
        if (message && message.includes('\n')) {
            ...
            // Try to parse remaining parts as JSON context
            if (parts.length > 1) {
                try {
                    const jsonPart = parts.slice(1).join('\n').trim();
                    if (jsonPart.startsWith('{')) {
                        parsedContext = JSON.parse(jsonPart);
                    }
                } catch {
        ...
        this.addLogEntry(level, cleanMessage, source, parsedContext);
    }
     ...
     */
    addLogEntry(level, message, source, context = null) {
        const logEntry = {
