...
/**
 * Update queue item download progress and status
 * @param {number} itemId - Queue item ID
 * @param {number} progress - Download progress (0-100)
 * @param {string} status - Download status
 * @param {boolean} prefetched - Whether item is prefetched
 * @returns {boolean} True if updated
 */
function updateQueueItemProgress(itemId, progress, status, prefetched = false) {
    const db = getDatabase();
    const result = db.prepare(`
        UPDATE queue_items 
